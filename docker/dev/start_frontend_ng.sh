#!/bin/bash
echo "[i] Script (NOT TESTED) to run the frontend-ng in dev mode"
...
docker-compose exec -d timesketch yarn run --cwd=/usr/local/src/timesketch/timesketch/frontend-ng build --mode development --watch
docker exec -it $CONTAINER_ID sh -c "cd /usr/local/src/timesketch/timesketch/frontend-ng; npm install; yarn install" >> /dev/null
